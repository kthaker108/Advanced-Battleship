...
class Ship:
    ...
    def isSunk(self, playerAims):
        if self.direction == 'horizontal':
            row = int((self.y - 50) // (self.height/2))
            col = int((self.x - 150) // (self.height))
    ...
    def drawIcons(self, left, top, cellWidth, cellHeight, icons):
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                if (row, col) == self.game.upgrades.lastBouncerHit and self.game.upgrades.showBounceAnimation:
                    drawRect(cellLeft, cellTop, cellWidth, cellHeight, fill='purple', opacity=60)
...
class Game:
    ...
    def setupBoard(self):
        ...
        #used this article to better understand spacing and formatting but did not include any code: https://www.uiprep.com/blog/everything-you-need-to-know-about-spacing-layout-grids 
        ...
        self.playerBoard = Board(self.rows, self.cols, self)
        self.computerBoard = Board(self.rows, self.cols, self)
    ...
    def drawLabels(self):
        ...
        number = 1
        for col in range(self.cols):
            drawLabel(str(number), self.boardLeft1 + (col * self.cellWidth) + 15, self.boardTop1 - 15)

            if self.computerShipsPlaced:
                drawLabel(str(number), self.boardLeft2 + (col * self.cellWidth) + 15, self.boardTop2 - 15)
            ...
            while shipInfo['placed'] == False and currAttempts < maxAttempts:
                ...
                for i in range(-1, shipInfo['size'] + 1):
                    for j in range(-1, 2):
                        ...
                        if ((dRow >= 0) and (dRow < self.rows) and (dCol >=0) and (dCol < self.cols) and 
                            (self.computerBoard.mainBoard[dRow][dCol] != None)):
                            isValidPlace = False
                            break
    ...
    def computerTurn(self):
        ...
        if self.wantToSink and self.sunkList:
            ...
            if ((row >=0) and (row < self.rows) and (col >= 0) and (col < self.cols) and (self.playerBoard.computerAims[row][col] == None)):
                ...
                if (row, col) in self.upgrades.bouncerCells:
                    ...
                    if isinstance(self.computerBoard.mainBoard[row][col], str):
                        self.computerBoard.playerAims[row][col] = 'X'
                    else:
                        self.computerBoard.playerAims[row][col] = 'O'
                    return
                ...
                if shipInCell != None:
                    ...
                    if shipName in self.upgrades.shieldedShips:
                        ...
                        self.playerBoard.computerAims[row][col] = None
                        return
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                if (self.playerBoard.computerAims[row][col] == None):
    ...
    def checkAllPlayerShipsSunk(self):
        ...
        print("All player ships sunk - game ending") 
        self.gameOver = True
        self.playerWon = False
        setActiveScreen('gameOver')
        return True
    ...
    def checkAdjacentCells(self, row, col):
        ...
        for drow, dcol in moves:
            newRow = row + drow
            newCol = col + dcol
            if ((newRow >= 0) and (newRow < self.rows) and (newCol >= 0) and (newCol < self.cols) and 
                (self.playerBoard.computerAims[newRow][newCol] == None)):
        ...
        for drow in [-1, 0, 1]:
            for dcol in [-1, 0, 1]:
                ...
                if 0 <= newRow < self.game.rows and 0 <= newCol < self.game.cols:
                    ...
                    if board.mainBoard[newRow][newCol] != None:
...
def ships_redrawAll(app):
    ...
    if app.game.invalid:
        drawLabel("INVALID PLACEMENT", app.game.width/2, app.game.height/2, fill='red', bold=True, size=30, border='maroon')

    if app.game.allShipsPlaced:
        drawRect(600, app.game.height/2 + 50, 250, 75, fill='darkBlue')
        drawLabel('Confirm Placement', 720, app.game.height/2 + 80, size=20, bold=True, fill='white')
        ...
        if app.game.hoveredRow != None and (app.game.hoveredCol != None):
            ...
            for drow in [-1, 0, 1]:
                for dcol in [-1, 0, 1]:
                    ...
                    if newRow >= 0 and newRow < app.game.rows and newCol >= 0 and newCol < app.game.cols:
                        cellLeft = 275 + newCol * app.game.cellWidth
                        cellTop = 50 + newRow * app.game.cellHeight

                        drawRect(cellLeft, cellTop, app.game.cellWidth, app.game.cellHeight, fill='pink', opacity=40)

    if app.game.lastHit != None:
        ...
        if app.game.playerBoard.computerAims[row][col] == 'X':
            ...
            drawLabel(f"Computer hit at {letter}{number}!", app.game.width/2, app.game.height - 50, size=20, bold=True, fill='red')

        else:
            drawLabel(f"Computer missed at {letter}{number}", app.game.width/2, app.game.height - 50, size=20, bold=True, fill='blue')
    ...
    if app.game.upgrades.shieldInUse:
        ...
        drawLabel("SHIELD MODE: Click a ship to protect it!", app.width/2, 30, size=24, bold=True, fill='white')
    ...
    for row, col in app.game.upgrades.bouncerCells:
        ...
        drawImage(app.game.bouncerIcon, cellLeft + app.game.cellWidth/4, cellTop + app.game.cellHeight/4,
                  width=app.game.cellWidth/2, height=app.game.cellHeight/2)
    ...
    if app.game.upgrades.showBounceAnimation and app.game.upgrades.lastBouncerHit:
        ...
        drawRect(cellLeft, cellTop, app.game.cellWidth, app.game.cellHeight, fill='purple', opacity=60)
        ...
        for col in range(board.cols):
            ...
            if isinstance(board.mainBoard[row][col], Ship):
                ...
                if ship.name in app.game.upgrades.shieldedShips:
                    ...
                    drawRect(cellLeft, cellTop, shieldWidth, shieldHeight, fill=None, border='yellow', borderWidth=3)
                    break
...
def ships_onMousePress(app, mouseX, mouseY):
    ...
    for ship in app.game.allShips:
        if not ship.placed:  
            ...
            if (mouseX >= ship.x and mouseX <= ship.x + shipWidth and mouseY >= ship.y and mouseY <= ship.y + shipHeight):
                app.game.draggingShip = ship
                app.game.dragOffsetX = mouseX - ship.x
                app.game.dragOffsetY = mouseY - ship.y
                break

    if ((app.game.computerShipsPlaced) and (mouseX >= app.game.boardLeft2) and (mouseX <= app.game.boardWidth + app.game.boardLeft2) and 
        (mouseY >= app.game.boardTop2) and (mouseY <= app.game.boardHeight + app.game.boardTop2)):
    ...
    if ((app.game.allShipsPlaced) and (mouseX >= 600) and (mouseX <= 850) and 
        (mouseY >= app.game.height/2 + 50) and (mouseY <= app.game.height/2 + 125)):
...
def ships_onMouseRelease(app, mouseX, mouseY):
    if app.game.draggingShip:
        ...
        inBoard = ((mouseX >= app.game.boardLeft1) and (mouseX <= app.game.boardLeft1 + app.game.boardWidth) and
...
def cellinMousePlayer(app, mouseX, mouseY):
    if ((mouseX >= app.game.boardLeft1) and (mouseX <= app.game.boardLeft1 + app.game.boardWidth) and
        ...
        if row >= 0 and row < app.game.rows and col >= 0 and col < app.game.cols:
            return (row, col)
        
    return(None, None)

def cellinMouse(app, mouseX, mouseY):
    if (mouseX >= 275 and mouseX <= 275 + app.game.boardWidth and mouseY >= 50 and mouseY <= 50 + app.game.boardHeight):
...
def fire_redrawAll(app):
    ...
    for row in range(app.game.rows):
        for col in range(app.game.cols):
            ...
            if app.game.computerBoard.playerAims[row][col] == 'X':
                ...
                drawImage(app.game.explosionIcon, cellLeft + (app.game.cellWidth/2) - 15, cellTop + (app.game.cellHeight/2) - 15)

            elif app.game.computerBoard.playerAims[row][col] == 'O':
                drawImage(app.game.waveIcon, cellLeft + (app.game.cellWidth/2) - 15, cellTop + (app.game.cellHeight/2) - 15)
    
    drawRect(275, 50, app.game.boardWidth, app.game.boardHeight, fill=None, border='steelBlue', borderWidth=2*app.game.cellBorderWidth)
    ...
    if app.game.upgrades.planeBombInUse:
        ...
        for row, col in app.game.upgrades.planeSelectedCells:
            ...
            drawRect(cellLeft, cellTop, app.game.cellWidth, app.game.cellHeight, fill='red', opacity=40)

            drawImage(app.game.planeBombIcon, cellLeft + app.game.cellWidth /4, cellTop + app.game.cellHeight / 4, 
                      width = app.game.cellWidth/2, height = app.game.cellHeight/2)
        
        remaining = 3 - len(app.game.upgrades.planeSelectedCells)
        drawLabel(f'PLANE BOMB: SELECT {remaining} MORE', app.width/2, 450, size=20, bold=True, fill='red')
...
def fire_onMousePress(app, mouseX, mouseY):
    ...
    if app.game.upgrades.planeBombInUse:
        if ((r, c) not in app.game.upgrades.planeSelectedCells and app.game.computerBoard.playerAims[r][c] is None):
        ...
        if app.game.computerBoard.playerAims[r][c] != None:
            return
            ...
            if shipName not in app.game.hitShips:
                ...
                for row in range(app.game.rows):
                    for col in range(app.game.cols):
                        ...
                        if (app.game.computerBoard.mainBoard[row][col] == shipName and app.game.computerBoard.playerAims[row][col] != 'X'):
                            allCellsHit = False
                            break
